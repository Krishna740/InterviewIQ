...
export const claimFreeCredits = async (req, res) => {
  try {
    ...
    if (user.freeCreditsClaimedAt) {
      return res.status(400).json({ 
        message: "You have already claimed your free credits once. You can only claim them once per account." 
      });
    }

    // Add 5000 free credits
    user.credits += 5000;
    user.freeCreditsClaimedAt = new Date();
    await user.save();
    ...
    res.json({
      success: true,
      message: "5000 free credits added successfully!",
      user: user,
    });
  } catch (error) {
    return res.status(500).json({ message: `Failed to claim free credits: ${error.message}` });
  }
}
